...
def start(update, context):
    text = 'Привет {}! Это бот, который показывает тебе свободные слоты по времени в студии Чайка. С помощью команды /choose_room выбирай репетиционную комнату. С помощью команды /choose_day выбирай день. И команда /show_results покажет тебе свободные сеты на текущую и следующую недели!)'.format(
        update.message.chat.first_name)
    command_keyboard = ReplyKeyboardMarkup(
        [['/choose_room'], ['/choose_day'], ['/choose_week'], ['/show_results'], ['/help']])
...
def choose_week(update, context):
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    update.message.reply_text(
        'ну если хочешь, можешь выбраь неделю)', reply_markup=reply_markup)
